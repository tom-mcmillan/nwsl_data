...
class FBRefPlayerExtractor:
    ...
    def extract_match_player_stats(self, html_content: str, match_id: str) -> Optional[List[Dict]]:
        """
        ...
        try:
            ...
            player_stats_tables = [tid for tid in table_ids if 'stats_' in tid and any(keyword in tid for keyword in ['summary', 'passing', 'defense', 'possession', 'misc', 'passing_types'])]
    ...
    def _extract_summary_stats(self, table, team_id: str, match_id: str) -> Dict[str, Dict]:
        ...
        try:
            ...
            if not player_col:
                logger.warning(f"⚠️  Could not find player column in summary table")
                return {}
    ...
    def process_html_file(self, html_file_path: str) -> bool:
        ...
        try:
            ...
            if player_stats:
                logger.info(f"✅ Successfully extracted comprehensive stats for {len(player_stats)} players in match {match_id}")
