...
from ..core.analytics_engine import NWSLAnalyticsEngine
...
class NWSLDataVisualizationAgent:
            ...
            fig.update_layout(
                polar=dict(
                    radialaxis=dict(
                        ...
                        range=[
                            0,
                            max(max(values), max(comparison_values) if comparison_values else 0) * 1.1,
                        ],
                    )
                ),
                showlegend=True,
    ...
    def _create_comparison_bar_chart(
        ...
        try:
            ...
            sorted_data = sorted(zip(entities, values, strict=False), key=lambda x: x[1], reverse=True)
            sorted_entities, sorted_values = zip(*sorted_data, strict=False)
            ...
            if horizontal:
                ...
                fig.update_layout(xaxis_title=metric_name, yaxis_title="", height=max(400, len(entities) * 30))
            else:
                fig.add_trace(
                    go.Bar(
                        x=sorted_entities,
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        ...
                        showscale=True,
                        sizemode="diameter",
                        sizeref=2.0 * max(size_values) / (40.0**2) if size_values else 1,
                        sizemin=4,
                    ),
    ...
    async def create_intelligent_visualization(
        self, user_query: str, data: dict[str, Any], context: str
    ) -> dict[str, Any]:
        """
