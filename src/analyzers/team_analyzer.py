...
class TeamPerformanceAnalyzer:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                h2h_matches = pd.read_sql_query(
                    ...
                    """,
                    conn,
                    params=[team_ids[team1_name], team_ids[team2_name]],
                )
                ...
                return {
                    ...
                    "record": {
                        team1_name: {
                            "wins": team1_wins,
                            "goals_for": h2h_matches["team1_goals"].sum(),
                            "goals_against": h2h_matches["team2_goals"].sum(),
                        },
                        team2_name: {
                            "wins": team2_wins,
                            "goals_for": h2h_matches["team2_goals"].sum(),
                            "goals_against": h2h_matches["team1_goals"].sum(),
                        },
                        "draws": draws,
                    },
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                top_scorers = pd.read_sql_query(
                    ...
                    """,
                    conn,
                    params=[team_id, season_id, limit],
                )
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                recent_matches = pd.read_sql_query(
                    ...
                    """,
                    conn,
                    params=[team_id, season_id, last_n_matches],
                )
    ...
    def compare_teams_in_season(self, team1_name: str, team2_name: str, season_id: int) -> dict:
        ...
        return {
            "season": season_id,
            "team_comparison": {team1_name: team1_summary, team2_name: team2_summary},
...
logger.info(
    "📖 Usage: analyze_team_season('Orlando Pride', 2025) or compare_teams('Orlando Pride', 'Chicago Stars FC', 2025)"
)
