...
def analyze_html_file(html_file_path):
    ...
    player_stats_tables = [tid for tid in table_ids if 'stats_' in tid and any(keyword in tid for keyword in ['summary', 'passing', 'defense', 'possession', 'misc'])]
...
def analyze_sample_files(html_dir, num_files=3):
    ...
    for i, html_file in enumerate(sorted(html_files)[:num_files]):
