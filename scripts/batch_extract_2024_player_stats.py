#!/usr/bin/env python3
"""
Batch extract comprehensive player statistics for all 2024 NWSL matches.
"""
...
def get_2024_matches(db_path):
    """Get all 2024 matches that need player stats extraction."""
...
def check_existing_extractions(db_path):
    ...
    cursor.execute("""
        ...
        FROM match_player 
        WHERE match_id IN (
            SELECT match_id FROM match WHERE season_id = 2024
        )
    """)
...
def batch_extract_player_stats(db_path):
    """Extract player stats for all 2024 matches."""
    
    # Get all 2024 matches
    all_matches = get_2024_matches(db_path)
    logging.info(f"Found {len(all_matches)} matches in 2024")
    ...
    if not matches_to_process:
        logging.info("All 2024 matches already have player stats extracted!")
        return
...
def get_extraction_summary(db_path):
    ...
    print(f"""
    ====== 2024 PLAYER STATS EXTRACTION SUMMARY ======
    Total 2024 matches: {total_matches}
...
def main():
    ...
    print("Starting batch extraction of 2024 player stats...")
