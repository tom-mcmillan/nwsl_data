#!/usr/bin/env python3
"""
Batch NWSL Match Player Statistics Scraper
Processes multiple match files from raw_match_pages directory
"""

import os
import sqlite3
from match_player_scraper import MatchPlayerScraper
import glob
...
def check_existing_matches(db_path):
    """Get list of match_ids that already have player data"""
    conn = sqlite3.connect(db_path)
    cursor = conn.execute('SELECT DISTINCT match_id FROM match_player')
...
def main():
    ...
    print(f"📋 Found {len(existing_matches)} matches with existing player data")
    ...
    for html_file in html_files[:100]:  # Process first 100 files
        ...
        try:
            success = scraper.scrape_match_player_stats(html_file, match_id)
            if success:
                processed += 1
