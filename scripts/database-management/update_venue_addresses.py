...
def update_venue_addresses(
    csv_path="/Users/thomasmcmillan/projects/nwsl_data/Soccer_Venues___Addresses.csv",
    ...
    for _, row in df.iterrows():
        ...
        for venue_id, db_venue_name, _current_address in db_venues:
        ...
        if not matched_venue_id:
            for venue_id, db_venue_name, _current_address in db_venues:
                ...
                if csv_venue_name.lower() in db_venue_name.lower() or db_venue_name.lower() in csv_venue_name.lower():
                    matched_venue_id = venue_id
                    match_type = "fuzzy"
                    break
    ...
    # Summary
    logging.info(f"\n{'='*50}")
    logging.info("VENUE ADDRESS UPDATE SUMMARY")
    logging.info(f"{'='*50}")
