...
class HTMLTeamStatsExtractor:
    ...
    def extract_team_stats_from_html(self, html_content: str, match_id: str) -> Optional[Dict[str, Dict[str, any]]]:
        """
        ...
        try:
            ...
            if team_stats_extra:
                ...
                if len(team_headers) >= 2:
                    ...
                    away_team_name = self._clean_team_name(team_headers[2].get_text().strip()) if len(team_headers) > 2 else self._clean_team_name(team_headers[1].get_text().strip())
                else:
    ...
    def _extract_category_stats(self, row, category: str) -> Optional[List[str]]:
    ...
    def _extract_detailed_stats(self, team_stats_div) -> Optional[Dict[str, Dict[str, int]]]:
        ...
        try:
            ...
            return {
                'home': home_stats,
                'away': away_stats
            } if home_stats or away_stats else None
            
        except Exception as e:
            logger.error(f"❌ Error extracting detailed stats: {str(e)}")
            return None
    
    def _normalize_stat_name(self, stat_name: str) -> Optional[str]:
    ...
    def _map_category_to_column(self, category: str) -> Optional[str]:
