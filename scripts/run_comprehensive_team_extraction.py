...
from extract_comprehensive_team_stats import extract_comprehensive_team_stats_from_match, save_comprehensive_team_stats_to_database
...
def main():
        ...
        try:
            ...
            if team_stats:
                ...
                for stats in team_stats:
                    non_null_fields = sum(1 for field in ['goals', 'fouls', 'touches_def_3rd', 'short_passes_completed', 'tackles_def_3rd'] 
                ...
                logging.info(f"✓ Match {match_id}: {len(team_stats)} teams processed ({'COMPREHENSIVE' if has_comprehensive else 'basic'})")
            else:
                failed_matches += 1
    ...
    logging.info(f"Comprehensive coverage: {comprehensive_coverage} ({100*comprehensive_coverage/successful_matches:.1f}% of successful)")
    ...
    logging.info(f"  📊 Basic performance (goals): {completeness[0]}/{total_records} ({100*completeness[0]/total_records:.1f}%)")
    logging.info(f"  ⚠️ Discipline (fouls): {completeness[1]}/{total_records} ({100*completeness[1]/total_records:.1f}%)")
    logging.info(f"  🏃 Possession detail: {completeness[2]}/{total_records} ({100*completeness[2]/total_records:.1f}%)")
    ...
    if top_teams:
        ...
        for team_name, matches, goals, pass_acc, takeon_rate, aerial_rate in top_teams:
            ...
            logging.info(f"    ⚽ {goals:.2f} goals/match, 📋 {pass_acc:.1f}% pass acc, 🏃 {takeon_rate:.1f}% takeon success, ✈️ {aerial_rate:.1f}% aerial wins")
