#!/usr/bin/env python3
"""
Import 2021 NWSL CSV to match_inventory
"""
...
class Import2021CSV:
    ...
    def import_csv_to_match_inventory(self, csv_path='data/excel_sheets/2021.csv'):
        """Import 2021 CSV data to match_inventory table"""
        print("📊 Starting 2021 CSV import to match_inventory...")
        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                try:
                    ...
                    # Determine match type ID and name - expanded playoff types for 2021
                    if match_type == 'Regular Season':
                        match_type_id = 'mt_a1b2c3d4'
                        match_type_name = 'Regular Season'
                    elif match_type in ['First round', 'Quarterfinals', 'Semifinals', 'Final']:
                    ...
                    # Insert into match_inventory with xG data
                    conn.execute('''
                        INSERT OR REPLACE INTO match_inventory 
                        (match_id, match_date, home_team_id, away_team_id, season_id, 
                         match_type_id, match_type_name, filename, extraction_status, xg_home, xg_away)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        match_id,
                        match_date,
                        ...
                        match_type_id,
                        match_type_name,
                        f"2021_csv_{match_id}.csv",  # filename reference
                    ...
                    print(f"  ✅ {match_id}: {home_name} vs {away_name} ({match_date}) - {match_type_name} {xg_display}")
        ...
        print(f"\n🎉 2021 CSV import complete!")
        print(f"   ✅ Successfully imported: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📊 Total 2021 matches in inventory: {successful}")
    ...
    def _build_team_mapping(self, conn):
        ...
        # Add 2021-specific mappings for team name variations
        # OL Reign is the same as Seattle Reign FC
        mapping['OL Reign'] = '257fad2b'  # Seattle Reign FC
        mapping['Reign'] = '257fad2b'     # Seattle Reign FC
        
        # Racing Louisville is a new team - check if it exists in database
        mapping['Louisville'] = 'da19ebd1'  # Racing Louisville
        mapping['Racing Louisville'] = 'da19ebd1'  # Racing Louisville
    ...
    def verify_import(self):
        ...
        print("🔍 Verifying 2021 import...")
        
        conn = sqlite3.connect(self.db_path)
        
        # Count 2021 matches by type
        counts = conn.execute('''
        ...
        total_2021 = sum(count for _, count in counts)
        
        print(f"   📊 2021 Season Summary:")
        for type_name, count in counts:
            print(f"      {type_name}: {count} matches")
        print(f"      Total: {total_2021} matches")
        
        # Show sample matches with xG data
        for type_name, _ in counts:
            samples = conn.execute('''
                SELECT match_id, match_date, 
                       ...
                       xg_home, xg_away
                FROM match_inventory 
                WHERE season_id = 2021 AND match_type_name = ?
                ORDER BY match_date
                LIMIT 3
            ''', (type_name,)).fetchall()
