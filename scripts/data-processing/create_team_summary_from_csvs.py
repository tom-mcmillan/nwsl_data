    ...
    try:
        ...
        try:
            ...
            if team_stats["passes_attempted"] > 0:
                team_stats["pass_accuracy"] = round(
                    (team_stats["passes_completed"] / team_stats["passes_attempted"]) * 100, 1
                )
            else:
            ...
            if team_stats["take_ons_attempted"] > 0:
                team_stats["take_on_success_rate"] = round(
                    (team_stats["take_ons_successful"] / team_stats["take_ons_attempted"]) * 100, 1
                )
            else:
        ...
        for summary_file in summary_files:
            ...
            try:
                ...
                values = (
                    ...
                    team_stats.get("shot_accuracy", 0.0),
...
def validate_data(db_path: str):
    ...
    for row in sample_data:
        logging.info(
            f"   Match {row[0][:8]}, Team {row[1][:8]} on {row[2]}: {row[3]} goals, {row[4]} shots, {row[6]:.1f}% pass accuracy"
        )
