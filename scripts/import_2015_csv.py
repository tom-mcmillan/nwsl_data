#!/usr/bin/env python3
"""
Import 2015 NWSL CSV to match_inventory
"""
...
class Import2015CSV:
    ...
    def import_csv_to_match_inventory(self, csv_path='data/excel_sheets/2015.csv'):
        """Import 2015 CSV data to match_inventory table"""
        print("📊 Starting 2015 CSV import to match_inventory...")
        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                try:
                    ...
                    match_date = self._parse_excel_date(date_str)
                    if not match_date:
                    ...
                    ''', (
                        ...
                        match_type_id,
                        match_type_name,
                        f"2015_csv_{match_id}.csv",  # filename reference
        ...
        print(f"\n🎉 2015 CSV import complete!")
        print(f"   ✅ Successfully imported: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📊 Total 2015 matches in inventory: {successful}")
    ...
    def _parse_excel_date(self, date_str):
        ...
        try:
            ...
            # Excel's epoch starts at January 1, 1900 (but actually 1899-12-30 due to Excel bug)
            excel_serial = int(float(date_str))
            
            # Excel date calculation (accounting for the 1900 leap year bug)
            if excel_serial > 59:  # After Feb 28, 1900
                excel_serial -= 1  # Adjust for Excel's leap year bug
            
            # Calculate days since epoch
            excel_epoch = date(1899, 12, 30)  # Excel's actual epoch
            actual_date = excel_epoch.fromordinal(excel_epoch.toordinal() + excel_serial)
            
            return actual_date.strftime('%Y-%m-%d')
            
        except Exception as e:
            print(f"⚠️ Excel date parsing error for '{date_str}': {e}")
            return None
    ...
    def _build_team_mapping(self, conn):
        ...
        # Add 2015-specific team name mappings
        mapping.update({
    ...
    def verify_import(self):
        ...
        print("🔍 Verifying 2015 import...")
        
        conn = sqlite3.connect(self.db_path)
        
        # Count 2015 matches by type
        counts = conn.execute('''
        ...
        total_2015 = sum(count for _, count in counts)
        
        print(f"   📊 2015 Season Summary:")
        for type_name, count in counts:
            print(f"      {type_name}: {count} matches")
        print(f"      Total: {total_2015} matches")
        ...
        for type_name, _ in counts:
            samples = conn.execute('''
                ...
                FROM match_inventory 
                WHERE season_id = 2015 AND match_type_name = ?
                ORDER BY match_date
                LIMIT 3
            ''', (type_name,)).fetchall()
