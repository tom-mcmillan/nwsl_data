#!/usr/bin/env python3
"""
Import 2025 NWSL CSV to match_inventory
"""
...
class Import2025CSV:
    ...
    def import_csv_to_match_inventory(self, csv_path='data/excel_sheets/2025.csv'):
        """Import 2025 CSV data to match_inventory table"""
        print("📊 Starting 2025 CSV import to match_inventory...")
        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                try:
                    ...
                    # Determine match type ID and name - expanded playoff types for 2025
                    ...
                    ''', (
                        ...
                        match_type_id,
                        match_type_name,
                        f"2025_csv_{match_id}.csv",  # filename reference
        ...
        print(f"\n🎉 2025 CSV import complete!")
        print(f"   ✅ Successfully imported: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📊 Total 2025 matches in inventory: {successful}")
    ...
    def _build_team_mapping(self, conn):
        ...
        # Add 2025-specific mappings - continuing from 2024
        mapping['Bay FC'] = '231a532f'      # Bay FC (second season)
        mapping['Royals'] = 'd4c130bc'      # Utah Royals (continuing)
        
        # Continue existing mappings from 2024
    ...
    def verify_import(self):
        ...
        print("🔍 Verifying 2025 import...")
        
        conn = sqlite3.connect(self.db_path)
        
        # Count 2025 matches by type
        counts = conn.execute('''
        ...
        total_2025 = sum(count for _, count in counts)
        
        print(f"   📊 2025 Season Summary (Partial Season):")
        for type_name, count in counts:
            print(f"      {type_name}: {count} matches")
        print(f"      Total: {total_2025} matches")
        ...
        for type_name, _ in counts:
            samples = conn.execute('''
                SELECT match_id, match_date, 
                       ...
                       xg_home, xg_away, wk
                FROM match_inventory 
                WHERE season_id = 2025 AND match_type_name = ?
                ORDER BY match_date
                LIMIT 3
            ''', (type_name,)).fetchall()
        ...
        # Show date range for partial season
        ...
        print(f"      First match: {first_date}")
        print(f"      Last match: {last_date}")
        print(f"      Note: Partial season - resumes August 2nd")
