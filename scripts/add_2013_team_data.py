...
def extract_2013_keeper_stats(keeper_csv_path: str) -> dict:
    ...
    try:
        ...
        if keeper_stats['shots_on_target_against'] > 0:
            keeper_stats['save_percentage'] = round((keeper_stats['saves'] / keeper_stats['shots_on_target_against']) * 100, 1)
        else:
        ...
        for summary_file in summary_files:
            ...
            try:
                ...
                values = (
                    ...
                    team_stats.get('penalty_goals', 0), team_stats.get('penalty_attempts', 0),
                    ...
                    team_stats.get('shots_on_target_against', 0), team_stats.get('saves', 0),
...
def validate_2013_data(db_path: str):
    ...
    for row in sample_data:
        logging.info(f"   Match {row[0][:8]} on {row[2]}: {row[3]} goals, {row[4]} shots ({row[5]} on target), {row[6]} saves/{row[7]} shots against ({row[8]}%)")
