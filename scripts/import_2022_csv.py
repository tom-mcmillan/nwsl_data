#!/usr/bin/env python3
"""
Import 2022 NWSL CSV to match_inventory
"""
...
class Import2022CSV:
    ...
    def import_csv_to_match_inventory(self, csv_path='data/excel_sheets/2022.csv'):
        """Import 2022 CSV data to match_inventory table"""
        print("📊 Starting 2022 CSV import to match_inventory...")
        ...
        with open(csv_path, 'r', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                try:
                    ...
                    # Determine match type ID and name - expanded playoff types
                    ...
                    ''', (
                        ...
                        match_type_id,
                        match_type_name,
                        f"2022_csv_{match_id}.csv",  # filename reference
        ...
        print(f"\n🎉 2022 CSV import complete!")
        print(f"   ✅ Successfully imported: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📊 Total 2022 matches in inventory: {successful}")
    ...
    def _build_team_mapping(self, conn):
        ...
        # Add 2022-specific mappings for new/renamed teams
        mapping['Angel City'] = 'ae38d267'  # Angel City FC
        mapping['Wave'] = 'bf961da0'        # San Diego Wave FC
        mapping['Current'] = '6f666306'     # Kansas City Current (same as Kansas City)
        
        # Continue existing mappings
    ...
    def verify_import(self):
        ...
        print("🔍 Verifying 2022 import...")
        
        conn = sqlite3.connect(self.db_path)
        
        # Count 2022 matches by type
        counts = conn.execute('''
        ...
        total_2022 = sum(count for _, count in counts)
        
        print(f"   📊 2022 Season Summary:")
        for type_name, count in counts:
            print(f"      {type_name}: {count} matches")
        print(f"      Total: {total_2022} matches")
        ...
        for type_name, _ in counts:
            samples = conn.execute('''
                SELECT match_id, match_date, 
                       ...
                       xg_home, xg_away
                FROM match_inventory 
                WHERE season_id = 2022 AND match_type_name = ?
                ORDER BY match_date
                LIMIT 3
            ''', (type_name,)).fetchall()
