#!/usr/bin/env python3
"""
Import 2019 NWSL CSV to match_inventory
"""
...
class Import2019CSV:
    ...
    def import_csv_to_match_inventory(self, csv_path='data/excel_sheets/2019.csv'):
        """Import 2019 CSV data to match_inventory table"""
        print("📊 Starting 2019 CSV import to match_inventory...")
            ...
            for row in reader:
                try:
                    ...
                    ''', (
                        ...
                        match_type_id,
                        match_type_name,
                        f"2019_csv_{match_id}.csv",  # filename reference
        ...
        print(f"\n🎉 2019 CSV import complete!")
        print(f"   ✅ Successfully imported: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📊 Total 2019 matches in inventory: {successful}")
    ...
    def verify_import(self):
        ...
        print("🔍 Verifying 2019 import...")
        
        conn = sqlite3.connect(self.db_path)
        
        # Count 2019 matches by type
        counts = conn.execute('''
        ...
        total_2019 = sum(count for _, count in counts)
        
        print(f"   📊 2019 Season Summary:")
        for type_name, count in counts:
            print(f"      {type_name}: {count} matches")
        print(f"      Total: {total_2019} matches")
        ...
        for type_name, _ in counts:
            samples = conn.execute('''
                SELECT match_id, match_date, 
                       ...
                       xg_home, xg_away
                FROM match_inventory 
                WHERE season_id = 2019 AND match_type_name = ?
                ORDER BY match_date
                LIMIT 3
            ''', (type_name,)).fetchall()
