#!/usr/bin/env python3
"""
Import 2018 NWSL CSV to match_inventory
"""
...
class Import2018CSV:
    ...
    def import_csv_to_match_inventory(self, csv_path='data/excel_sheets/2018.csv'):
        """Import 2018 CSV data to match_inventory table"""
        print("📊 Starting 2018 CSV import to match_inventory...")
            ...
            for row in reader:
                try:
                    ...
                    ''', (
                        ...
                        match_type_id,
                        match_type_name,
                        f"2018_csv_{match_id}.csv",  # filename reference
        ...
        print(f"\n🎉 2018 CSV import complete!")
        print(f"   ✅ Successfully imported: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📊 Total 2018 matches in inventory: {successful}")
    ...
    def _parse_date(self, date_str):
        ...
        try:
            # Handle formats like "3/24/18"
            month, day, year = date_str.split('/')
            
            # Convert 2-digit year to 4-digit (18 -> 2018)
            if len(year) == 2:
                year = f"20{year}"
    ...
    def verify_import(self):
        ...
        print("🔍 Verifying 2018 import...")
        
        conn = sqlite3.connect(self.db_path)
        
        # Count 2018 matches by type
        counts = conn.execute('''
        ...
        total_2018 = sum(count for _, count in counts)
        
        print(f"   📊 2018 Season Summary:")
        for type_name, count in counts:
            print(f"      {type_name}: {count} matches")
        print(f"      Total: {total_2018} matches")
        ...
        for type_name, _ in counts:
            samples = conn.execute('''
                ...
                FROM match_inventory 
                WHERE season_id = 2018 AND match_type_name = ?
                ORDER BY match_date
                LIMIT 3
            ''', (type_name,)).fetchall()
