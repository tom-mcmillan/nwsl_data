        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                return {
                    ...
                    "season_match_counts": dict(zip(seasons['season_id'], seasons['matches'])),
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for _, team in teams.iterrows():
                    seasons = pd.read_sql_query("""
                        ...
                        """, conn, params=[team['team_id']])
                    
                    results.append({
                        "team_id": team['team_id'],
                        "primary_name": team['team_name_1'],
                        "all_names": [n for n in [team['team_name_1'], team['team_name_2'], team['team_name_3'], team['team_name_4']] if n],
    ...
    def validate_user_query(self, team_name: str = None, season_id: int = None) -> Dict:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                # Check season
                if season_id:
                    season_exists = pd.read_sql_query("""
                        SELECT COUNT(*) as count FROM match WHERE season_id = ?
                        """, conn, params=[season_id]).iloc[0]['count'] > 0
                ...
                if team_name:
                    ...
                    if not team_exists:
                        ...
                        similar_teams = pd.read_sql_query("""
                            ...
                            ORDER BY team_name_1 
                            LIMIT 3
                            """, conn, params=[f'%{team_name.split()[0]}%'])['team_name_1'].tolist()
