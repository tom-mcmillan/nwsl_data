...
def extract_comprehensive_team_stats_from_match(
    match_id,
    ...
    for file_path in csv_files:
        ...
        if "_stats_" in filename and not filename.endswith("_all.csv") and not filename.startswith("sched_"):
            ...
            if len(parts) >= 4:
                ...
                if len(team_id) >= 8 and stat_type not in ["stats"]:  # Valid team IDs are 8+ char hashes
    ...
    for team_id, files in team_files.items():
        ...
        try:
            ...
            if stats_data.get("short_passes_attempted", 0) > 0:
                stats_data["pass_accuracy_short"] = (
                    stats_data.get("short_passes_completed", 0) / stats_data["short_passes_attempted"]
                ) * 100

            if stats_data.get("medium_passes_attempted", 0) > 0:
                stats_data["pass_accuracy_medium"] = (
                    stats_data.get("medium_passes_completed", 0) / stats_data["medium_passes_attempted"]
                ) * 100

            if stats_data.get("long_passes_attempted", 0) > 0:
                stats_data["pass_accuracy_long"] = (
                    stats_data.get("long_passes_completed", 0) / stats_data["long_passes_attempted"]
                ) * 100

            # Take-on success rate
            if stats_data.get("take_ons_attempted", 0) > 0:
                stats_data["take_on_success_rate"] = (
                    stats_data.get("take_ons_successful", 0) / stats_data["take_ons_attempted"]
                ) * 100
...
def main():
    ...
    if team_stats:
        ...
        for stats in team_stats:
            ...
            print(
                f"⚽ Basic: {stats.get('goals', 0)} goals, {stats.get('shots', 0)} shots ({stats.get('shots_on_target', 0)} on target)"
            )
            print(
                f"📋 Passing: {stats.get('passes_completed', 0)}/{stats.get('passes_attempted', 0)} ({stats.get('pass_accuracy', 0):.1f}%)"
            )
            print(
                f"   - Short: {stats.get('short_passes_completed', 0)}/{stats.get('short_passes_attempted', 0)} ({stats.get('pass_accuracy_short', 0):.1f}%)"
            )
            print(
                f"   - Long: {stats.get('long_passes_completed', 0)}/{stats.get('long_passes_attempted', 0)} ({stats.get('pass_accuracy_long', 0):.1f}%)"
            )
            print(
                f"🏃 Possession: {stats.get('touches', 0)} touches, {stats.get('carries', 0)} carries ({stats.get('carries_distance', 0)} yards)"
            )
            print(
                f"🛡️ Defense: {stats.get('tackles', 0)} tackles ({stats.get('tackle_success_rate', 0):.1f}% success), {stats.get('interceptions', 0)} interceptions"
            )
            print(
                f"⚠️ Discipline: {stats.get('fouls', 0)} fouls, {stats.get('yellow_cards', 0)} yellows, {stats.get('corners', 0)} corners"
            )
            print(
                f"✈️ Aerials: {stats.get('aerials_won', 0)}-{stats.get('aerials_lost', 0)} ({stats.get('aerial_win_rate', 0):.1f}% win rate)"
            )
    else:
