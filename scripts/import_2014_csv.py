#!/usr/bin/env python3
"""
Import 2014 NWSL CSV to match_inventory
"""
...
class Import2014CSV:
    ...
    def import_csv_to_match_inventory(self, csv_path='data/excel_sheets/2014.csv'):
        """Import 2014 CSV data to match_inventory table"""
        print("📊 Starting 2014 CSV import to match_inventory...")
            ...
            for row in reader:
                try:
                    ...
                    ''', (
                        ...
                        match_type_id,
                        match_type_name,
                        f"2014_csv_{match_id}.csv",  # filename reference
        ...
        print(f"\n🎉 2014 CSV import complete!")
        print(f"   ✅ Successfully imported: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📊 Total 2014 matches in inventory: {successful}")
    ...
    def _parse_date(self, date_str):
        ...
        try:
            # Handle formats like "4/12/14"
            month, day, year = date_str.split('/')
            
            # Convert 2-digit year to 4-digit (14 -> 2014)
            if len(year) == 2:
                year = f"20{year}"
    ...
    def verify_import(self):
        ...
        print("🔍 Verifying 2014 import...")
        
        conn = sqlite3.connect(self.db_path)
        
        # Count 2014 matches by type
        counts = conn.execute('''
        ...
        for type_name, _ in counts:
            samples = conn.execute('''
                ...
                FROM match_inventory 
                WHERE season_id = 2014 AND match_type_name = ?
                ORDER BY match_date
                LIMIT 3
            ''', (type_name,)).fetchall()
