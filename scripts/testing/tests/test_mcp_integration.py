...
from mcp_server_config import create_mcp_server, handle_tool_call
...
def test_tool_integration():
    ...
    result = handle_tool_call("analyze_team_season", {"team_name": "North Carolina Courage", "season_id": 2025})
    if "error" not in result:
        data = json.loads(result["content"])
    ...
    result = handle_tool_call("validate_query_parameters", {"team_name": "North Carolina Courage", "season_id": 2025})
    if "error" not in result:
        data = json.loads(result["content"])
