#!/usr/bin/env python3
"""
Import 2017 NWSL CSV to match_inventory
"""
...
class Import2017CSV:
    ...
    def import_csv_to_match_inventory(self, csv_path='data/excel_sheets/2017.csv'):
        """Import 2017 CSV data to match_inventory table"""
        print("📊 Starting 2017 CSV import to match_inventory...")
            ...
            for row in reader:
                try:
                    ...
                    ''', (
                        ...
                        match_type_id,
                        match_type_name,
                        f"2017_csv_{match_id}.csv",  # filename reference
        ...
        print(f"\n🎉 2017 CSV import complete!")
        print(f"   ✅ Successfully imported: {successful}")
        print(f"   ❌ Failed: {failed}")
        print(f"   📊 Total 2017 matches in inventory: {successful}")
    ...
    def _parse_date(self, date_str):
        ...
        try:
            # Handle formats like "4/15/17"
            month, day, year = date_str.split('/')
            
            # Convert 2-digit year to 4-digit (17 -> 2017)
            if len(year) == 2:
                year = f"20{year}"
    ...
    def _build_team_mapping(self, conn):
        ...
        # Add 2017-specific team name mappings
        mapping.update({
            ...
            'Courage': '5f911568'         # North Carolina Courage (replaced WNY Flash)
        })
    ...
    def verify_import(self):
        ...
        print("🔍 Verifying 2017 import...")
        
        conn = sqlite3.connect(self.db_path)
        
        # Count 2017 matches by type
        counts = conn.execute('''
        ...
        total_2017 = sum(count for _, count in counts)
        
        print(f"   📊 2017 Season Summary:")
        for type_name, count in counts:
            print(f"      {type_name}: {count} matches")
        print(f"      Total: {total_2017} matches")
        ...
        for type_name, _ in counts:
            samples = conn.execute('''
                ...
                FROM match_inventory 
                WHERE season_id = 2017 AND match_type_name = ?
                ORDER BY match_date
                LIMIT 3
            ''', (type_name,)).fetchall()
